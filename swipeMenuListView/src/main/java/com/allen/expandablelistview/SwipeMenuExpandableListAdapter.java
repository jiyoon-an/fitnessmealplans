...
public class SwipeMenuExpandableListAdapter implements ExpandableListAdapter, OnSwipeItemClickListenerForExpandable {
    ...
    public void notifyDataSetChanged(boolean ifKeepMenuOpen) {
        ...
        if (i >= 0 && tv != null
                && (mList.getParent() instanceof LinearLayout || mList.getParent() instanceof LinearLayout)) {
            ...
            if (mList.getParent() instanceof LinearLayout) {
                ...
                ((LinearLayout.LayoutParams) lpNew).setMargins(lp.leftMargin, sLo[1] - fLo[1], lp.rightMargin, 0);
            } else if (mList.getParent() instanceof LinearLayout) {
                LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mList.getLayoutParams();
                lpNew = new LinearLayout.LayoutParams(tv.getWidth(), tv.getHeight());
                ((LinearLayout.LayoutParams) lpNew).setMargins(lp.leftMargin, sLo[1] - fLo[1], lp.rightMargin, 0);
            }
            v = new View(mContext);
